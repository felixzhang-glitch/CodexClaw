...
function extractFilePayload(item) {
  ...
  const url = String(payload.url || payload.cdn_url || payload.download_url || payload.file_url || "").trim();
  if (!url) return null;
  return {
    url,
    name: String(payload.file_name || payload.name || payload.title || "").trim(),
    aesKey: String(payload.aes_key || payload.aeskey || "").trim(),
    size: Number(payload.file_size || payload.size || 0),
  };
}
