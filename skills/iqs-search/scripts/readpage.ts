...
/**
 * IQS ReadPage Script
 * Usage: bun readpage.ts --url "https://example.com" [options]
 */
 ...
 */
function parseArgs(args: string[]): ReadPageOptions {
 ...
 */
async function readPage(options: ReadPageOptions): Promise<FormattedContent> {
 ...
 * Format page content
 */
function formatContent(data: PageData | undefined, format: string): FormattedContent {
  if (!data) {
    return {
      title: null,
      url: null,
